...
export class AppComponent {
  ...
  constructor(private user: LoginService) {
    this.user.getData().subscribe(data => {
      ...
      // data["data"].foreach(readValues());

      for (var i = 1; i <= data["data"].length; i++) {
        resultSet[i].id = data["data"][i].id;
        resultSet[i].first_name = data["data"][i].first_name;
        resultSet[i].last_name = data["data"][i].last_name;
        resultSet[i].avatar = data["data"][i].avatar;
      }
